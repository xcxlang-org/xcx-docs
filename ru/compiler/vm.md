# Виртуальная машина XCX (VM) — v3.1

Виртуальная машина XCX — **регистровый** runtime для байткода XCX с трассовым JIT на Cranelift.

## Архитектура

- **Файл**: `src/backend/vm.rs`
- **Модель выполнения**: цикл Fetch-Decode-Execute (`execute_bytecode`) над плоским набором регистров
- **Регистры**: `Vec<Value>` на кадр, индексы слотов `u8`
- **Глобальные**: один `Vec<Value>` за `Arc<RwLock<Vec<Value>>>`, общий для всех воркеров
- **JIT**: `src/backend/jit.rs` — нативный код для горячих трасс
